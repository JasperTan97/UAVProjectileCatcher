...
void isolate_ball(std::vector<PointData, EigenPointData> &Frame, float& ball_x, float& ball_y, float& ball_z){
	...
	// cout << "xsize: " << xs.size() << ", ysize: " << ys.size() << endl;
	// FINDING STDDEV
	// Find sum
	...
	for (auto i = Frame.begin(); i != Frame.end(); i++){
		if (true || i->x >= mean_x - 2*stdev_x && i->x <= mean_x + 2*stdev_x &&
			i->y >= mean_y - 2*stdev_y && i->y <= mean_y + 2*stdev_y)
			temp.push_back(*i);
	...
	// Swap the pointers
	Frame.swap(temp); 
	//newx.clear(); newy.clear(); newz.clear(); 
}
